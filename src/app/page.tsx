...
import { exportToJSON, shareToClipboard, generateShareText } from '@/utils/exportUtils';
...
export default function ConicSectionsPage() {
  ...
  const [mathematicalElements, setMathematicalElements] = useState<MathematicalElement[]>([]);
  ...
  useEffect(() => {
    ...
    let planeColor: number, planePosition: [number, number, number], planeRotation: [number, number, number];
  ...
  useEffect(() => {
    ...
    if (conicType === 'parabola') {
      ...
      elements.push(
        ...
        { type: 'point', x: focus.x, y: focus.y, label: 'Focus', color: '#ef4444' },
        { type: 'line', y: directrixY, label: 'Directrix', color: '#10b981' }
      );
    } else if (conicType === 'circle') {
      elements.push(
        { type: 'point', x: hEllipse, y: kEllipse, label: 'Center', color: '#ef4444' }
      );
    } else if (conicType === 'ellipse') {
      ...
      elements.push(
        { type: 'point', x: hEllipse, y: kEllipse, label: 'Center', color: '#3b82f6' },
        { type: 'point', x: foci[0].x, y: foci[0].y, label: 'Focus₁', color: '#ef4444' },
        { type: 'point', x: foci[1].x, y: foci[1].y, label: 'Focus₂', color: '#ef4444' }
      );
      
      if (isHorizontal) {
        elements.push(
          { type: 'point', x: hEllipse - aEllipse, y: kEllipse, label: 'Vertex', color: '#10b981' },
          { type: 'point', x: hEllipse + aEllipse, y: kEllipse, label: 'Vertex', color: '#10b981' }
        );
      } else {
        elements.push(
          { type: 'point', x: hEllipse, y: kEllipse - bEllipse, label: 'Vertex', color: '#10b981' },
          { type: 'point', x: hEllipse, y: kEllipse + bEllipse, label: 'Vertex', color: '#10b981' }
        );
      }
    } else if (conicType === 'hyperbola') {
      const foci = calculateHyperbolaFoci(aEllipse, bEllipse, hEllipse, kEllipse);
    ...
    setMathematicalElements(elements.filter(el => Math.abs(el.x || 0) <= 15 && Math.abs(el.y || 0) <= 15));
  ...
  const handleExportEquation = () => {
    ...
    const parameters = conicType === 'parabola' 
      ? { a, h, k }
  ...
  const handleExportImage = () => {
    ...
    ctx.fillText(`Type: ${conicType.charAt(0).toUpperCase() + conicType.slice(1)}`, 20, 55);
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${themeClasses}`} onClick={(e) => {
      ...
      <div className="flex h-screen">
        ...
        <div className="w-1/3 p-6 overflow-y-auto border-r border-gray-300 dark:border-gray-700">
          ...
          <div className="mb-6">
            <div className={`flex rounded-lg p-1 w-full ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
            ...
            <div className={`text-center mt-2 text-sm font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              {conicType.charAt(0).toUpperCase() + conicType.slice(1)} Conic Type
            </div>
          </div>

          {/* Equation Display */}
          <div className={`p-4 rounded-lg mb-6 border ${darkMode ? 'bg-blue-900/20 border-blue-800' : 'bg-blue-50 border-blue-200'}`}>
            <h3 className={`text-lg font-semibold mb-2 ${darkMode ? 'text-blue-300' : 'text-blue-800'}`}>
              Current Equation:
            </h3>
            <div className={`text-lg font-mono ${darkMode ? 'text-blue-200' : 'text-blue-900'}`}>
              {getEquation()}
            </div>
          </div>
